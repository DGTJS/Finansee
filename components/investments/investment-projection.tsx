...
export function InvestmentProjection({ initialCents = 0 }: { initialCents?: number }) {
  ...
  const values = useMemo(() => {
    ...
    const points = Array.from({ length: 13 }, (_, index) => { const elapsedMonths = Math.round((months * index) / 12); const pointMonths = Math.min(months, elapsedMonths); return { label: index === 0 ? "Hoje" : unit === "years" ? `${Math.max(elapsedMonths / 12, 1)}a` : `${Math.max(pointMonths, 1)}m`, without: futureValue(initialValue, 0, pointMonths, rate), with: futureValue(initialValue, monthlyValue, pointMonths, rate) }; });
