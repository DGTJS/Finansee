...
import { cancelTransaction, cancelTransactions } from "@/server/actions/transactions";
...
export function TransactionsPage({ initialItems, options, spaceId = "personal-space" }: { initialItems: Item[]; options: TransactionOptions; spaceId?: string }) {
  ...
  async function cancelMany(ids: string[]) { const result = await cancelTransactions(ids, spaceId); if (result.success) setItems((current) => current.map((item) => ids.includes(item.id) ? { ...item, status: "cancelled" } : item)); return result; }
