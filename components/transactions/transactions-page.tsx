...
export function TransactionsPage({ initialItems, options, spaceId = "personal-space" }: { initialItems: Item[]; options: TransactionOptions; spaceId?: string }) {
  ...
  async function cancelMany(ids: string[]) { const result = await cancelTransactions(ids, spaceId); if (result.success) setItems((current) => current.filter((item) => !ids.includes(item.id))); return result; }
