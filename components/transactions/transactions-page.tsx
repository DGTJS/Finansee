...
import { cancelTransaction } from "@/server/actions/transactions";
...
export function TransactionsPage({ initialItems, options, spaceId = "personal-space" }: { initialItems: Item[]; options: TransactionOptions; spaceId?: string }) {
  ...
  function cancel(id: string) { startTransition(async () => { const result = await cancelTransaction(id, spaceId); if (result.success) setItems((current) => current.map((item) => item.id === id ? { ...item, status: "cancelled" } : item)); }); }
