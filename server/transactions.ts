...
export async function getTransactions(filters?: {
  ...
  const rows = await prisma.transaction.findMany({
    ...
    include: {
      account: { include: { owner: { select: { image: true } } } },
  ...
  const resolved = rows.map((row) => ({
    ...
    status: resolveTransactionStatus(
      row.status as "pending" | "paid" | "overdue" | "cancelled",
    ...
    accountOwnerImage:
      row.account.type === "credit_card"
        ? (row.account.owner?.image ?? null)
        : null,
    categoryName: row.category?.name ?? null,
  }));
  return filters?.status && filters.status !== "all"
    ? resolved.filter((row) => row.status === filters.status)
    : resolved;
}
...
export async function getTransactionOptions(spaceId = "personal-space") {
  ...
  const [accounts, categories] = await Promise.all([
    prisma.financialAccount.findMany({
      where: { financialSpaceId: spaceId, archivedAt: null },
  ...
  return {
    accounts: accounts.map(({ id: value, name: label, type, bank }) => ({
      value,
      label,
      type,
      bank,
    })),
    categories: categories.map(({ id: value, name: label, kind }) => ({
      value,
      label,
      kind,
    })),
